...
<body>
	...
	<hr>
	<form action="<%=TMSView.PACKAGE_LIST_CTL%>" method="post"
			...
			<div class="row">
				...
				<div class="col-sm-4">
					<input class="btn btn-outline-primary btn-md m-0 waves-effect"
						...
						value="<%=PackageListCtl.OP_SEARCH%>"> Or <input
		...
		<table id="tablePreview"
			...
			<tbody>
				...
				<tr>
					...
					<td><img alt="" height="100px" width="170px" src="<%=TMSView.IMAGE_FOLDER+bean.getImage()%>"></td>
					<td><%=bean.getName()%></td>
					<td><%=bean.getType()%></td>
					<td><%=bean.getPrice()%></td>
					<td><%=bean.getFeature()%></td>
					<td><%=bean.getDetail()%></td>
					<td><%=bean.getLocation()%></td>
					<td><a class="btn btn-outline-primary btn-sm m-0 waves-effect"
						href="package?id=<%=bean.getId()%>">Edit</a></td>
				</tr>
				<%
					}
				%>
			...
			<tbody>
				<tr>
					<td><input type="submit" name="operation"
						...
						value="<%=PackageListCtl.OP_PREVIOUS%>"
						<%=(pageNo == 1) ? "disabled" : ""%>></td>

					<td><input type="submit" name="operation"
						class="btn btn-outline-primary btn-sm m-0 waves-effect"
						value="<%=PackageListCtl.OP_NEW%>"></td>

					<td><input type="submit" name="operation"
						class="btn btn-outline-primary btn-sm m-0 waves-effect"
						value="<%=PackageListCtl.OP_DELETE%>"
						<%=(list.size() == 0) ? "disabled" : ""%>></td>

					<td><input type="submit" name="operation"
						class="btn btn-outline-primary btn-sm m-0 waves-effect"
						value="<%=PackageListCtl.OP_NEXT%>"
